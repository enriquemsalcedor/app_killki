...
public class AdminSQLiteOpenHelper  extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("create table configuracion(mensaje text, intervalo integer, sensor integer, num_veces integer, mensaje_voz text, estatus text)");
        db.execSQL("create table organizacion(nombre text, email text, direccion text, telefono text, logo text, img text)");
        db.execSQL("create table historial(fecha text, hora text, estatus text)");


        db.execSQL("INSERT INTO configuracion (mensaje, intervalo, sensor, num_veces, mensaje_voz, estatus) VALUES ('¡NECESITO AYUDA!', 0, 3, 0, '', '0')");
